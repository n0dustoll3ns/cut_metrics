...
; действует и в движке саммари). Тултип веса —
 ...
 7, фикс-раунд №3 (2026-09-16):** линия EMA непрерывна на коротких пропусках
 дня, слот пропускается) и рвётся только при 5+ пустых днях; средние всех метрик
 саммари) считаются только по данным после
 5+-дневного пропуска своей метрики — `daysAfterLastGap` (`gap_rule.dart`),
 каждой метрике отдельно; покрытие «по N дн.» честно уменьшается; в движке после
 меньше 3 точек → «Недостаточно данных»; график веса показывает все сегменты.
 198 зелёных (+8), analyze — базлайн.
