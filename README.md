...
- `_ChartCard` — общий виджет для всех графиков: isLoading/isEmpty/legend/child
- `LegendItem` — цветная точка + подпись
- Графики: `LineChart` (вес+EMA), `BarChart` (питание), `BarChart` (стековый сон)
...
- `NutritionDay` заполняется из Health Connect, но график питания показывает только `calories` одним столбцом — макронутриенты (protein/fat/carbs) загружаются, но не отображаются (закомментированный код в `_buildEnergyBalanceBarChart`)
