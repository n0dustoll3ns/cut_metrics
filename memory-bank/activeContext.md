...
**Фаза 7 — дизайн выполнен (2026-09-11), задание на реализацию готово, код не начат.**
Дизайн: `docs/design-system.html` §05 (паттерн «Энергобаланс», карточка «Питание», тёмные
превью в §06) и `docs/screen-today-graph-summary-settings.html` (все 4 экрана + тёмный фрейм +
аннотации «Фаза 7»). Реализация — следующим чатом по `docs/phase7_implementation_task.md`
(порядок A → B → C, промежуточная сборка после A). Решения 2026-09-11 — спека §0 пп. 13–14:
дельта v2 динамическая (вес × |цель−факт| × 11, коридор 50–300, округление до 10); оверрайд
«Шаги» — хранится коэффициент, UI «ккал/1000 шагов» по последнему резолвленному весу.
Допущение дизайна: avgBalance — по дням с приходом И расходом, покрытие — по приходу;
подсказка профиля — под карточкой «Питание» на «Сегодня».
