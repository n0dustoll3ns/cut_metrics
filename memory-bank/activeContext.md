...
№1 закрывается fallback-детектом sourceId + идемпотентной записью delete-then-write);
тёмная тема «приборная» тёмно-серая через ThemeExtension, themeMode system/light/dark,
дефолт system; AccessGate перенесён в финальную фазу «Монетизация». От пользователя
ожидается DebugLog с устройства (сценарий — задание №2, A0) для точечной диагностики
бага ручного ввода.
